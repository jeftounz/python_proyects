...
#Para resolver ese problemas sobre los Joins
"""for product in Product.select(Product.title, Category.title
                              ).join(ProductCategory
                                     ).join(Category, on=(ProductCategory.category == Category.id)):
    if product.categories:
        print(product.title, '-', product.categories[0].title)
    else:
        print(product.title, '-', 'No category')"""
